...
analyze table uber_orc compute statistics;
analyze table uber_orc compute statistics for columns;
EOF
...
sqrt( pow(sub1.long - sub2.long, 2) + pow(sub1.lat - sub2.lat, 2) ) <= (1/(111 * 25));
EOF

cat<<EOF>hotspotend.sql
select id, long, lat, count(*) c from (
select sub1.id as id, sub2.id as id2, sub1.long, sub1.lat, sub2.long long2, sub2.lat lat2, sqrt( pow(sub1.long - sub2.long, 2) + pow(sub1.lat - sub2.lat, 2) ) * 111 as distance_km
from
( select id, long, lat, row_number() over ( partition by id order by ts desc ) as rn from uber_orc ) sub1,
( select id, long, lat, row_number() over ( partition by id order by ts desc ) as rn from uber_orc ) sub2
where sub1.rn = 1 and sub2.rn = 1 and
sqrt( pow(sub1.long - sub2.long, 2) + pow(sub1.lat - sub2.lat, 2) ) <= (1/(111 * 10)) ) sub group by id, long, lat order by c;
EOF

cat<<EOF>hotspotstart.sql
select id, long, lat, count(*) c from (
select sub1.id as id, sub2.id as id2, sub1.long, sub1.lat, sub2.long long2, sub2.lat lat2, sqrt( pow(sub1.long - sub2.long, 2) + pow(sub1.lat - sub2.lat, 2) ) * 111 as distance_km
from
( select id, long, lat, row_number() over ( partition by id order by ts ) as rn from uber_orc ) sub1,
( select id, long, lat, row_number() over ( partition by id order by ts ) as rn from uber_orc ) sub2
where sub1.rn = 1 and sub2.rn = 1 and
sqrt( pow(sub1.long - sub2.long, 2) + pow(sub1.lat - sub2.lat, 2) ) <= (1/(111 * 10)) ) sub group by id, long, lat order by c;
EOF
